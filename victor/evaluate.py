...
from victor.losses import build_eps2d
...
class MetricBundle(NamedTuple):
    """Per-profile scalar evaluation metrics (v8)."""
    profile_idx     : int
    psnr            : float
    cc              : float
    rel_err         : float
    proj_mse        : float
    symmetry_ratio  : float   # var(a0) / total variance — 1=symmetric
    poloidal_energy : float   # mean squared harmonic amplitude


class EvalBundle(NamedTuple):
    """Full per-profile evaluation outputs (v8)."""
    profile_idx : int
    coeffs      : np.ndarray   # (N_RADIAL, NC)  harmonic coefficients
    eps_pred    : np.ndarray   # (N_GRID, N_GRID)  true 2D field via build_eps2d
...
def compute_symmetry_ratio(coeffs: np.ndarray) -> float:
    """
    Fraction of emissivity variance in the symmetric a0 channel.
    1.0 = fully symmetric, lower = harmonics contributing.
    """
    var_a0  = float(np.var(coeffs[:, 0]))
    var_har = float(np.sum(np.var(coeffs[:, 1:], axis=0)))
    return float(var_a0 / (var_a0 + var_har + 1e-12))


def compute_poloidal_energy(coeffs: np.ndarray) -> float:
    """Mean squared harmonic amplitude. 0 = no asymmetry."""
    return float(np.mean(coeffs[:, 1:] ** 2))
...
def compute_all_metrics(
    ...
    return MetricBundle(
        ...
        symmetry_ratio  = compute_symmetry_ratio(coeffs),
        poloidal_energy = compute_poloidal_energy(coeffs),
    )
...
def evaluate_profile(
    ...
    w_ops,
    grids,
    rho_graph = None,   # kept for API compat — unused in v8
) -> EvalBundle:
    """
    Run a full v8 forward pass on one profile.

    v8 model: (g, psi_flat, rho_flat_eq, xi) -> coeffs (N_RADIAL, NC)
    build_eps2d() expands coeffs onto the 2D pixel grid.
    """
    # ── Forward pass ─────────────────────────────────────────────────
    coeffs_jnp = model.apply(
        params,
        profile["g_ideal"],
        profile["psi_n"],
        grids.RHO_FLAT,
    ...
    coeffs_np = safe_numpy(coeffs_jnp)                  # (N_RADIAL, NC)

    # ── Build true 2D emissivity via polar harmonics ──────────────────
    ...
    N           = cfg.N_GRID
    eps_pred_2d = safe_numpy(eps2d_flat).reshape(N, N)  # (N_GRID, N_GRID)

    # ── Ground truth ─────────────────────────────────────────────────
    eps_gt_2d = safe_numpy(profile["eps_n"])             # (N_GRID, N_GRID)

    # ── Projected sinogram ────────────────────────────────────────────
    g_pred_jax = w_ops.matvec(eps2d_flat)
...
def evaluate_all(
    ...
    for prof in profiles:
        ...
        print(
            ...
            f"sym={m.symmetry_ratio:.3f}  "
            f"polE={m.poloidal_energy:.5f}"
        )
        bundles.append(eb)
...
def plot_reconstruction_2d(
    ...
    """
    Four-panel 2D diagnostic (v8):
      [GT  |  VICTOR Pred  |  Residual  |  Poloidal Variance]
    """
    N      = cfg.N_GRID
    ...
    pred     = eb.eps_pred
    residual = pred - gt
    pol_var  = np.var(pred, axis=1, keepdims=True) * np.ones_like(pred)

    vmax = max(float(gt.max()), float(pred.max()), 1e-8)
    vabs = float(np.abs(residual).max()) + 1e-8

    fig, axes = plt.subplots(1, 4, figsize=(22, 5.0))
    panels = [
        (gt,       0,     vmax, cmap,      "ε [a.u.]",   "Ground Truth ε"),
        (pred,     0,     vmax, cmap,      "ε [a.u.]",   "VICTOR v8 Prediction"),
        (residual, -vabs, vabs, "seismic", "Δε",          "Residual (pred − GT)"),
        (pol_var,  0,     float(pol_var.max()) + 1e-8,
                               "viridis",  "var(ε)",      "Poloidal Variance"),
    ]

    for ax, (data, vmn, vmx, cm, clbl, title) in zip(axes, panels):
        _pcolormesh_with_cbar(ax, R_2d, Z_2d, data, vmn, vmx, cm, clbl)
        _rho_contour_overlay(ax, R_2d, Z_2d, rho_2d)
    ...
    m = eb.metrics
    suptitle = (
        f"VICTOR v8.0 — Profile {eb.profile_idx}"
        f"  PSNR={m.psnr:.1f} dB"
        f"  CC={m.cc:.3f}"
        f"  sym={m.symmetry_ratio:.3f}"
        f"  polE={m.poloidal_energy:.5f}"
...
def plot_gt_gallery(
    ...
    fig.suptitle("VICTOR v8.0 — Ground-Truth Emissivity (all profiles)",
...
def plot_pred_gallery(
    ...
    fig.suptitle("VICTOR v8.0 — VICTOR Reconstruction (all profiles)",
                 fontsize=14, fontweight="bold", y=1.01)
    return fig


# ══════════════════════════════════════════════════════════════════════
# 7a.  Centrepiece: GT vs VICTOR comparison grid (all profiles)
# ══════════════════════════════════════════════════════════════════════
...
def plot_comparison_grid(
    ...
    fig.suptitle(
        "VICTOR v8.0 — Ground Truth vs Reconstruction (all profiles)",
        fontsize=14, fontweight="bold", y=1.005,
    )
    return fig

# ══════════════════════════════════════════════════════════════════════
# 7b.  Harmonic profiles  — NEW in v8
# ══════════════════════════════════════════════════════════════════════

def plot_harmonic_profiles(eb: EvalBundle, grids) -> plt.Figure:
    """
    Per-harmonic radial profiles: a0, a1, b1, a2, b2.

    Shows which harmonics are active and their radial structure.
    a0 should be peaked and positive; harmonics should be small.
    """
    rho_rad = safe_numpy(grids.RHO_RADIAL)
    coeffs  = eb.coeffs                       # (N_RADIAL, NC)
    NC      = coeffs.shape[1]
    n_harm  = (NC - 1) // 2

    labels = ["a₀ (symmetric)"]
    for h in range(1, n_harm + 1):
        labels.append(f"a{h} (cos {h}θ)")
        labels.append(f"b{h} (sin {h}θ)")

    colors = plt.cm.tab10(np.linspace(0, 1, NC))
    fig, axes = plt.subplots(NC, 1, figsize=(8, 2.5 * NC), sharex=True)
    if NC == 1:
        axes = [axes]

    for i, (ax, label, color) in enumerate(zip(axes, labels, colors)):
        ax.plot(rho_rad, coeffs[:, i], lw=1.8, color=color, label=label)
        ax.axvline(1.0, ls=":", c="red", lw=0.8, alpha=0.7, label="LCFS")
        ax.axhline(0.0, ls="-", c="k",  lw=0.4, alpha=0.3)
    ...
    m = eb.metrics
    fig.suptitle(
        f"VICTOR v8.0 — Harmonic Profiles  Profile {eb.profile_idx}"
        f"  polE={m.poloidal_energy:.5f}  sym={m.symmetry_ratio:.3f}",
...
def plot_radial_profile(
    ...
    """
    1-D radial profile comparison (v8).

    Left  — a0 coefficient (symmetric emission) vs GT binned radially.
    Right — pixel-binned comparison of full 2D pred vs GT inside plasma.
    """
    rho_rad  = safe_numpy(grids.RHO_RADIAL)
    rho_flat = safe_numpy(grids.RHO_FLAT)
    a0       = eb.coeffs[:, 0]                 # (N_RADIAL,) symmetric component
    eps_gt_f = eb.eps_gt.flatten()
    eps_pr_f = eb.eps_pred.flatten()

    # Bin GT onto model radial axis
    ...
    ax1.plot(rho_rad, a0,        lw=1.8, color="darkorange",
             linestyle="--", label="VICTOR a₀(ρ)")
    ax1.axvline(1.0, color="grey", lw=0.8, linestyle=":", label="LCFS")
    ax1.set_xlabel("ρ", fontsize=11)
    ax1.set_ylabel("Emissivity [a.u.]", fontsize=11)
    ax1.set_title("Symmetric component a₀ vs GT", fontsize=11)
    ax1.legend(fontsize=10); ax1.set_xlim(0.0, cfg.RHO_MAX); ax1.grid(alpha=0.3)
    ...
    ax2.set_xlabel("ρ", fontsize=11)
    ax2.set_ylabel("Emissivity [a.u.]", fontsize=11)
    ax2.set_title("Pixel-binned 2D comparison (ρ < 1)", fontsize=11)
    ax2.legend(fontsize=10); ax2.set_xlim(0.0, 1.0); ax2.grid(alpha=0.3)

    fig.suptitle(
        f"VICTOR v8.0 — Radial Profile  Profile {eb.profile_idx}"
        f"  PSNR={m.psnr:.1f} dB  CC={m.cc:.3f}",
...
def plot_metrics_summary(eval_bundles: list) -> plt.Figure:
    ...
    fig.suptitle("VICTOR v8.0 — Evaluation Metrics Summary",
...
def plot_loss_curves(
    ...
    """
    KEYS   = ["total", "proj", "boundary", "smooth", "pol"]
    COLORS = ["black", "tab:orange", "tab:brown", "tab:green", "tab:purple"]
    TITLES = ["Total loss", "Projection (data fidelity)",
              "Boundary enforcement", "2D TV smoothness", "Poloidal reg"]
    ...
    fig.suptitle("VICTOR v8.0 — Training loss curves",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    if save_path:
...
def plot_all_profiles(
    ...
    for eb in eval_bundles:
        ...
        fig = plot_harmonic_profiles(eb, grids)
        path = os.path.join(results_dir, f"harmonic_profiles_{pid:03d}.png")
        fig.savefig(path, dpi=150, bbox_inches="tight")
        plt.close(fig)
        print(f"  Saved: {path}")
...
def save_summary_csv(eval_bundles: list, path: str) -> None:
    ...
    rows = [("profile_idx", "psnr_dB", "cc", "rel_err",
             "proj_mse", "symmetry_ratio", "poloidal_energy")]
    for eb in eval_bundles:
        m = eb.metrics
        rows.append((
            m.profile_idx,
            ...
            round(m.rel_err,          6),
            round(m.proj_mse,         8),
            round(m.symmetry_ratio,   6),
            round(m.poloidal_energy,  8),
        ))
...
if __name__ == "__main__":
    ...
    grids = SimpleNamespace(
        ...
        THETA_FLAT = np.arctan2(YY, XX).flatten().astype(np.float32),
        RHO_RADIAL = np.linspace(0.0, cfg.RHO_MAX, N_RADIAL).astype(np.float32),
    )

    coeffs  = rng.random((N_RADIAL, NC)).astype(np.float32)
    coeffs[:, 0] = np.abs(coeffs[:, 0])     # a0 >= 0
    eps_pred = rng.random((N, N)).astype(np.float32)
    eps_gt   = rng.random((N, N)).astype(np.float32)
    g        = rng.random(128).astype(np.float32)
    amask    = np.ones(128, dtype=np.float32)

    metrics = compute_all_metrics(
        eps_pred, eps_gt, g, g * 0.9, coeffs, amask, profile_idx=0
    )
    ...
    fig = plot_harmonic_profiles(eb, grids); plt.close(fig)
    print("plot_harmonic_profiles OK")
    ...
    hist_dummy = {
        ...
        "pol":   list(np.exp(-np.linspace(0, 5, 300)) * 0.01),
    }
    fig = plot_loss_curves(hist_dummy); plt.close(fig)
    print("plot_loss_curves OK")

    print("\nevaluate.py v8 self-test PASSED")