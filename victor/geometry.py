...
def make_W_operators(W_csr, device=None) -> WOperators:
    ...
    # matvec and vecmat are intentionally NOT wrapped with @jax.jit here.
    #
    # Rationale: trainer.py calls jax.set_mesh() at import time to register
    # a global multi-device mesh.  Any @jax.jit applied in this file would
    # be compiled under that ambient mesh context and would expect inputs
    # sharded across all devices — but these operators always receive plain
    # single-device arrays (shape (n_pixels,) on GPU:0).
    #
    # The correct approach is to leave these as un-JIT'd closures and let
    # the *outer* jax.jit in the training step (make_train_step) compile
    # them in the right context with the right input shardings.
    # For the one standalone call in data_loader.load_profiles (at load
    # time, outside the training step), the functions execute eagerly on
    # GPU via the captured W_IDX/W_LEN/W_MSK device arrays — no tracing,
    # no mesh conflict.
