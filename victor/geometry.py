...
#    via jax.device_put().  When None, a helper _resolve_device() falls
#    back to GPU:0 if available, otherwise the JAX default device.
#
#  * make_W_operators: W_IDX, W_LEN, W_MSK and the matvec/vecmat
#    closures are all placed on the resolved device.  This ensures
#    forward projection always dispatches on GPU during training.
#
...
def make_W_operators(W_csr, device=None) -> WOperators:
    ...
    closures are placed on `device` (GPU:0 by default).  This ensures
    forward projection always dispatches on GPU during training.
