...
def make_train_step(
    ...
    def _loss_one(p, g_clean_i, psi_i, rho_eq_i, sigma, rng_i, xi_i,
                  active_mask, rho_flat_pix, theta_flat, rho_radial):
        g_noisy = inject_noise_fn(g_clean_i, sigma, rng_i)
        coeffs  = model.apply(p, g_noisy, psi_i, rho_eq_i, xi_i)
        return loss_fn(
    ...
    def _loss_batch(p, g_clean, psi_batch, rho_eq_batch, sigma, rngs, xi,
                    active_mask, rho_flat_pix, theta_flat, rho_radial):
        batched = jax.vmap(
            lambda g_i, psi_i, rho_eq_i, rng_i, xi_i: _loss_one(
                p, g_i, psi_i, rho_eq_i, sigma, rng_i, xi_i,
                active_mask, rho_flat_pix, theta_flat, rho_radial,
            )
    ...
    def step_fn(
        ...
        def _loss_for_grad(p):
            return _loss_batch(
                p, g_clean, psi_batch, rho_eq_batch,
                sigma, rngs, xi,
                active_mask, rho_flat_pix, theta_flat, rho_radial,
            )
    ...
    print(
        f"make_train_step (v8): step_fn compiled  "
...
def train_one_profile(
    *,
    ...
    rho_eq_single = jax.device_put(rho_flat_pix,       _gpu)          # (N_GRID²,)
...
def train(
    *,
    ...
    print(f"\n{'='*60}")
    if single_profile:
        print(f"VICTOR v8.0 -- Single-Profile Training  (profile index {start_prof})")
    else:
        print(f"VICTOR v8.0 -- Training  ({n_prof} profiles x {n_stages} stages)")
