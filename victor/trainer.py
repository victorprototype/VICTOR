...
def replicate_params(params, opt_state, n_dev: int):
    ...
    # PositionalSharding is available in all recent JAX versions.
    # Reshape to (n_dev, 1, 1, ...) so only axis-0 is split across
    # devices; all weight axes are replicated.  This matches what pmap
    # expects: params[i] lives on devices[i].
    sharding = jax.sharding.PositionalSharding(devices)
    replicate = lambda x: jax.device_put(
        jnp.stack([x] * n_dev),
        sharding.reshape([n_dev] + [1] * x.ndim),
    )
    return (
        jax.tree_util.tree_map(replicate, params),
        jax.tree_util.tree_map(replicate, opt_state),
    )
...
def _shard(arr: jnp.ndarray, n_dev: int) -> jnp.ndarray:
    """
    Broadcast `arr` to all `n_dev` devices for pmap dispatch.
    ...
    Uses jax.sharding.PositionalSharding to split the stacked array's
    leading axis across devices, which is what pmap requires.
    jax.device_put_sharded is deprecated in this JAX version.

    When n_dev == 1 returns arr unchanged (jit path).
    """
    if n_dev == 1:
        return arr
    devices = jax.devices()[:n_dev]
    sharding = jax.sharding.PositionalSharding(devices)  # shape (n_dev,)
    # Reshape sharding to split only axis 0; all other axes are replicated.
    sharding = sharding.reshape([n_dev] + [1] * arr.ndim)
    return jax.device_put(jnp.stack([arr] * n_dev), sharding)
