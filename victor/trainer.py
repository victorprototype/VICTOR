...
def replicate_params(params, opt_state, n_dev: int):
    ...
    # device_put_sharded physically places replica i on devices[i].
    # jnp.stack (old code) kept everything on GPU:0, causing the
    # "sharding mismatch" ValueError from pmap.
    replicate = lambda x: jax.device_put_sharded([x] * n_dev, devices)
    return (
...
def _shard(arr: jnp.ndarray, n_dev: int) -> jnp.ndarray:
    ...
    jax.device_put_sharded physically places replica i on devices[i],
    giving each replica a SingleDeviceSharding that matches the leading
    axis pmap expects.  jnp.stack (old code) kept all replicas on GPU:0
    and caused a "sharding mismatch" ValueError.
    ...
    if n_dev == 1:
        return arr
    devices = jax.devices()[:n_dev]
    return jax.device_put_sharded([arr] * n_dev, devices)
