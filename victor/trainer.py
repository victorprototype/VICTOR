...
def replicate_params(params, opt_state, n_dev: int):
    ...
    # Equivalent replacement: stack first, then device_put with a
    # GSPMDSharding that places slice i on devices[i].
    sharding = jax.sharding.GSPMDSharding.get_replicated(devices)
    replicate = lambda x: jax.device_put(jnp.stack([x] * n_dev), sharding)
    return (
