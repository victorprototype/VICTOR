...
#  * PRNGKey placed on GPU before inject_noise to avoid CPU dispatch.
# ============================================================
...
def make_train_step(
    ...
    print("make_train_step: step_fn compiled with jax.jit.")
    return step_fn
...
def train_one_profile(
    *,
    ...
    g_clean     = jax.device_put(profile["g_ideal"], _gpu)
    xi          = jax.device_put(profile["xi"],      _gpu)
    ...
    for si, (s_ep, s_sig) in enumerate(stages):
        ...
        # PRNGKey on GPU — a CPU key forces inject_noise to run on CPU
        ...
        for ep in range(ep_start, s_ep):
            ...
            # Materialise to CPU only at log intervals
